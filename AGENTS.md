...
8. **Keep the Linear project in sync** — [Squatter](https://linear.app/ielyas/project/squatter-8016284756f8) (team Elyas, `E2-…`). Every TRACKER.md status change gets the matching Linear update (issue created / In Progress / In Review, milestone = phase). **Every plan in `plans/` gets an issue when the plan is written** (title `Plan 0NN — …`), In Progress on execute, id in the plan's index row; a `reconcile` pass checks Linear against the index, and a refused creation (the workspace hit its free-plan issue limit on 2026-09-01) is written into the row, never skipped quietly. Never set an issue to Done/Canceled unless the owner explicitly asks. Details: [rules/issue-tracker-status.md](rules/issue-tracker-status.md). (Plans clause added 2026-09-01 after r2tor shipped eleven plans with no issue; this repo's index shows 10 of 14 plans without one.)
